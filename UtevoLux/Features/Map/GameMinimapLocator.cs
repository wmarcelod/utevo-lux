...
public static class GameMinimapLocator
{
    ...
    /// <summary>The player's <c>minimapmarkers.bin</c> (their in-game map marks) path, or null.</summary>
    public static string? FindPlayerMarkersFile()
    {
        foreach (string dir in MinimapCandidates())
        {
            try
            {
                string file = Path.Combine(dir, "minimapmarkers.bin");
                if (File.Exists(file))
                    return file;
            }
            catch
